...
def is_valid_word(word, hand, word_list):
    ...
    # Condition 1: if word contains a '*' and if it's in the word_list
    k = word_lower.find('*')
    new_list = []
    if k == -1: # '*' not in list
        pass
    else:
        word_fh = word_lower[:k]
        word_sh = word_lower[k+1:]
        for i in word_list:
            if i.find(word_fh) == 0 and i.find(word_sh) == k+1 and i[k] in VOWELS:
                new_list.append(i)
        if len(new_list)>0:
            pass
        else:
    ...
    #Condition 2: Entirely composed of letters in the hand
    for i in word_lower:
        if hand_copy.get(i,0) <= 0:
            return False
        else:
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while len(hand) > 1:
        ...
        # If the word is valid:
            ...
            print(f'{word} earned {score_earned} points. ','Total: ', total_score)
                # Tell the user how many points the word earned,
                # and the updated total score
            # Otherwise (the word is not valid):
                # Reject invalid word (print a message)
        else:
            print('this is an invalid word, please input again???????')
